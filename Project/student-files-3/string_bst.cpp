...
string_bst::node_t* foo (string_bst::node_t* & curr,const tree_key &key)  {
    if (curr) {
        if (curr -> data.word == key)
            return curr;
        if (key < curr -> data.word)
            return foo(curr -> left,key);
        else
            return foo(curr -> right,key);
    }
    else return 0;
}
...
int string_bst::word_frequency(const tree_key &key) const {
    ...
    // ADD CODE HERE
    node_t* temp;
    node_t * curr = get_root();
    temp = foo(curr, key);
    if (temp != NULL)
        return temp->data.freq;
    else
        return 0;
}
