...
/*  //Answer for the Question 1;
    ...
    cout<< string(d, '*')<< to_string(heap[node]) << endl;
    d++;
...
*/
    ...
    cout << setw( 4* d)<<" "<< to_string(heap[node])<<endl;
...
int* mergeHeap(int* heap1, int* heap2, int size1, int size2) {
    ...
    for(int i =0; i < size1; i ++){                //When we run size1 times
        heap[i] = heap1[i];
    }

    for(int m = size1 ; m < size ; m ++){          //When we run size2 = (size - size1) times
        heap[m] = heap2[m];
        //swapDown(heap,m, size);
    }
    heapify(heap, size);                           //Finally we need to call heapify, we run O(n) times
