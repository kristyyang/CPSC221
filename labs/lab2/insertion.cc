...
void scootOver( int j ) {
  int k;
  for( k = y_size; k > j; k-- )                        //BUG02 : loop does not work properly,changed the  k size from k++ to k--;
    y[k] = y[ k-1 ];
}
...
void insert( int xx ) {
  ...
  for( j=0; j< y_size; j++ ) {
    if( xx < y[j] ) {
      ...
      y[j] = xx;
      return;
    }else if (xx > y[j]){                              //BUG3: needed else statement for when xx>y[jj]
            y[y_size] = xx;
  }
}
}
